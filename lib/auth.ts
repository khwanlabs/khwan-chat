...
// NextAuth config (mirrors khwan-dashboard/lib/auth.ts). The Google ID
// token is persisted onto the JWT and exposed on the session so the server-side
// proxy routes can forward it to the Khwan API as a Bearer token, which the
...
export const authOptions: NextAuthOptions = {
  ...
  cookies: {
    sessionToken: {
      name: "khwan-chat.session-token",
      options: { httpOnly: true, sameSite: "lax", path: "/", secure: process.env.NODE_ENV === "production" },
    },
    callbackUrl: {
      name: "khwan-chat.callback-url",
      options: { sameSite: "lax", path: "/", secure: process.env.NODE_ENV === "production" },
    },
    csrfToken: {
      name: "khwan-chat.csrf-token",
      options: { httpOnly: true, sameSite: "lax", path: "/", secure: process.env.NODE_ENV === "production" },
    },
  },
  callbacks: {
